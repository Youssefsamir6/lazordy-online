...
(function($) {
    ...
    const managerDiscount = parseFloat($('#id_manager_discount_amount').val()) || 0;
    const amountPaid = parseFloat($('#id_amount_paid').val()) || 0;

    const finalTotal = (grandTotal - discount - managerDiscount).toFixed(2);
    const remaining = (finalTotal - amountPaid).toFixed(2);

    $('#id_subtotal_amount').val(grandTotal.toFixed(2));
    $('.form-row.field-subtotal_amount .readonly').text(grandTotal.toFixed(2));

    $('#id_total_amount').val(finalTotal);
    $('.form-row.field-total_amount .readonly').text(finalTotal);

    $('#id_amount_remaining').val(remaining);
    $('.form-row.field-amount_remaining .readonly').text(remaining);

    console.log(`✅ Invoice updated — Subtotal: ${grandTotal.toFixed(2)}, Total: ${finalTotal}, Remaining: ${remaining}`);
}
        ...
        $(document).on('input change', '#id_manager_discount_amount', function() {
            updateInvoiceTotal();
        });

        $(document).on('input change', '#id_amount_paid', function() {
            updateInvoiceTotal();
        });
